...
(() => {
  ...
  // src/utils/component/swiper/swiperTech.ts
  init_live_reload();

  // node_modules/.pnpm/swiper@11.1.15/node_modules/swiper/swiper.mjs
  init_live_reload();
  ...
  // node_modules/.pnpm/swiper@11.1.15/node_modules/swiper/modules/index.mjs
  init_live_reload();
  ...
  // src/utils/component/swiper/swiperUsecases.ts
  init_live_reload();
  function swiperUsecasesCarousel() {
  ...
  // src/utils/global/darkMode.ts
  init_live_reload();
  function darkMode(cssVariables, duration, ease) {
    ...
    function setColors(colorObject, animate) {
      if (typeof gsapWithCSS !== "undefined" && animate) {
        gsapWithCSS.to(htmlElement, {
          ...colorObject,
          duration,
          ease
        });
  ...
  function parseColors(cssVariables, computed) {
    const lightColors = {};
    const darkColors = {};
  ...
  // src/utils/global/gsapTransition.ts
  init_live_reload();
  var gsapTransitionOut = () => {
  ...
  // src/utils/global/heroBackground.ts
  init_live_reload();
  function initHeroBackgroundHover() {
    ...
    components.forEach((component) => {
      ...
      if (!backgroundRound)
        return;
      const roundElement = backgroundRound;
      ...
      gsapWithCSS.set(backgroundRound, {
        x: initialX,
        y: 0
      });
      component.addEventListener("mousemove", (e) => {
        ...
        gsapWithCSS.to(backgroundRound, {
          x: centerX,
          y: centerY,
          duration: 1.5,
          ease: "power2.out"
  ...
  // src/utils/global/modalViewer.ts
  init_live_reload();
  ...
  var _defaults2 = extend3(defaults3, {});
  var SplitType = /* @__PURE__ */ function() {
    _createClass(SplitType2, null, [{
      ...
      value: function setDefaults(options) {
        _defaults2 = extend3(_defaults2, parseSettings(options));
        return defaults3;
      }
      /**
    ...
    function SplitType2(elements, options) {
      ...
      this.isSplit = false;
      this.settings = extend3(_defaults2, parseSettings(options));
  ...
  var activeSplitText = () => {
    ...
    const typeSplitAlternateFirstLetterOff = new SplitType(
      ...
      {
        types: "words",
        // Removed chars to only split words
        tagName: "span",
        charClass: "split-text-alternate"
      }
    );
    [
      [typeSplitFirstLetterOff, ".split-text"],
      [typeSplitAlternateFirstLetterOff, ".split-text-alternate"]
    ].forEach(([split2, selector3]) => {
      if (split2 instanceof SplitType && split2.words) {
        split2.words.forEach((word) => {
          if (split2 === typeSplitAlternateFirstLetterOff) {
            ...
            word.querySelectorAll(selector3)[0]?.classList.add("is-first");
          }
        });
      }
    });
  ...
  function initTechLineScroll() {
    ...
    gsapWithCSS.set(lineElements, { height: "0%" });
    ...
    gsapWithCSS.set(bulletElements, { scale: 0, opacity: 0 });
  ...
  // src/utils/usecases/lottieParallax.ts
  init_live_reload();
  gsapWithCSS.registerPlugin(ScrollTrigger2);
  function animateUsecasesAsset() {
  ...
  // src/utils/vision/expertise-tabs.ts
  init_live_reload();
  function animateButtonExpertise() {
    const startButtonAnimation = (button) => {
      gsapWithCSS.to(button, {
        backgroundImage: "conic-gradient(from 510deg, #9e144f, #fedbcd, #055d46, #b5413e, #c4671b, #9e144f)",
        duration: 4,
        ease: "none",
        repeat: -1,
        onUpdate: function() {
          ...
          button.style.background = `conic-gradient(from ${degrees}deg, #9e144f, #fedbcd, #055d46, #b5413e, #c4671b, #9e144f) border-box`;
        }
      });
    };
    const buttons = document.querySelectorAll(
      ".vision_expertise_tabs-link.w--current .vision_expertise_conic-background"
    );
    ...
    const observer = new MutationObserver((mutations) => {
      mutations.forEach((mutation) => {
        if (mutation.target instanceof Element) {
          ...
          const conicBackground = tabLink.querySelector(".vision_expertise_conic-background");
          if (conicBackground) {
            if (tabLink.classList.contains("w--current")) {
              startButtonAnimation(conicBackground);
            } else {
              gsapWithCSS.killTweensOf(conicBackground);
            }
          }
        }
      });
    });
    const tabLinks = document.querySelectorAll(".vision_expertise_tabs-link");
  ...
  // src/utils/vision/heroVideoAnimation.ts
  init_live_reload();
  function heroVideoAnimation() {
    let timeline2 = null;
    let videoLoadListeners = [];
    function init4() {
      ...
      if (!videoFrames1.length || !videoFrames2.length || !videoFrames3.length)
        return;
      Array.from(syncVideos).forEach((video) => {
  ...
  // src/utils/vision/rotationBackground.ts
  init_live_reload();
  function rotateBackgroundAssets() {
    const blurAssets = document.querySelectorAll(".background-blur-asset");
    blurAssets.forEach((asset) => {
      gsapWithCSS.to(asset, {
        rotationZ: 360,
        duration: 10,
        repeat: -1,
        ease: "none"
      ...
      transitions: [
        {
          ...
          async leave(data) {
            ...
            gsapWithCSS.to(data.current.container, { opacity: 0, duration: 0.5 });
          },
          async enter(data) {
            await gsapWithCSS.to(data.next.container, { opacity: 1, duration: 1 });
          }
        }
      ]
    });
